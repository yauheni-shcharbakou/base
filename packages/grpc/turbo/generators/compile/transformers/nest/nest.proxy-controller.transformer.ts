...
export class NestProxyControllerTransformer extends BaseTransformer {
      ...
      declarations: [
        {
          ...
          initializer: (writer) => {
            writer
              .writeLine(
                `(dtoSchema: ${this.names.dtoSchemaInterface}): GrpcProxyControllerFactory => `,
              )
              .inlineBlock(() => {
                writer
                  .writeLine(
                    'return (params: GrpcProxyControllerFactoryParams): GrpcProxyControllerFactoryResult => ',
                  )
                  .inlineBlock(() => {
                    writer
                      .writeLine(
                        'const { GrpcController, ValidateGrpcPayload, InjectGrpcService, proxyPipes } = params;',
                      )
                      .blankLine()
  ...
  canTransform(): boolean | Promise<boolean> {
    ...
    return !!(this.controllerInterface && this.clientInterface && this.serviceNameValue);
  }
