...
- **Abstract buses** → `@backend/event-bus/src/generated/index.ts`: a base `EventBus`, one abstract `<Host><Service>EventBus` per service with `emit<Event>(event)` + `emitMany<Event>(events)`, and the `EventBusHost` enum.
- **Nats adapter** (the only adapter, pug-templated) → `@backend/nats/src/generated/index.ts`: per-service `Nats<Host><Service>Transport` (event-pattern constants, a `.ControllerMethods()` class decorator, and `.EventBus` = the abstract class), subscriber interfaces `Nats…EventController`, cross-host handler interfaces `Nats…EventHandler`, and `NatsClientFactory` (maps each abstract bus → its concrete `NatsJetStreamClientProxy`-backed impl).

Subjects are kebab-cased `host-service-event` (`auth-user-create`); JetStream streams are `host-service-stream`. Both packages' `src/generated/` are committed — edit the strategy, `pnpm compile:event-bus`, then `build`.
...
- **Domain events** are emitted via injected abstract `@backend/event-bus` buses (e.g. `AuthUserEventBus.emitCreate`) after a successful write, and consumed by `interface/nats/*.controller.ts` subscribers — see *Event-bus codegen pipeline* above.
