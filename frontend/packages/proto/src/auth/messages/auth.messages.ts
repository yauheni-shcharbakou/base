...
export const AuthToken: MessageFns<AuthToken> = {
  encode(message: AuthToken, writer: BinaryWriter = new BinaryWriter()): BinaryWriter {
    ...
    if (message.expireDate !== undefined) {
      Timestamp.encode(toTimestamp(message.expireDate), writer.uint32(18).fork()).join();
    }
    return writer;
  },
...
export const AuthStreamCode: MessageFns<AuthStreamCode> = {
  ...
  decode(input: BinaryReader | Uint8Array, length?: number): AuthStreamCode {
    const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
    const end = length === undefined ? reader.len : reader.pos + length;
  ...
  create<I extends Exact<DeepPartial<AuthStreamCode>, I>>(base?: I): AuthStreamCode {
    return AuthStreamCode.fromPartial(base ?? ({} as any));
  },
